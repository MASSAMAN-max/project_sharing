...
// これまでは共有トークンだけで誰でも閲覧できたが、Googleアカウントでの
// ログインを必須にしたため、閲覧者のメールアドレスも一緒に送ってもらい、
// 認証チェックを通過した場合のみ案件データを返すようにした。
// メールアドレスをURL（クエリパラメータ）に乗せたくないため、
// GET→POSTに変更している（/api/verify-user 等と同じ方針）。
//
// 【2つのGASプロジェクトを呼び分けている点に注意】
// ①GAS_URL（案件共有GAS）        …トークンから案件データ本体を取得する。
// ②MAIN_APP_GAS_URL（メインアプリGAS）…メールアドレスの認証チェックのみ行う。
